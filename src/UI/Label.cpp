...
bool Label::initialize(std::tuple<float, float> position, std::tuple<float, float> size, std::wstring text, std::shared_ptr<GlyphsLoader> glyphs, std::shared_ptr<Shader> shader) {
	...
	changePositionProgressUp->addArgument(_entity->getComponent<TextComponent>(), "page");
	changePositionProgressUp->addArgument(_entity->getComponent<TextComponent>(), "totalPages");
	...
	changePositionProgressDown->addArgument(_entity->getComponent<TextComponent>(), "page");
	changePositionProgressDown->addArgument(_entity->getComponent<TextComponent>(), "totalPages");
...
bool Label::setScrollerProgress(std::shared_ptr<View> scrollerProgress) {
...
std::shared_ptr<Back> Label::getScrollerProgress() {
	for (auto view : _views) {
...
std::shared_ptr<View> LabelFactory::createView(std::string name) {
	...
	label->setScrollerProgress(std::dynamic_pointer_cast<Back>(_backFactory->createView("scrollerProgress")));
	return label;
}
